...
class IndexBar<T> extends StatefulWidget {
  final int Function(T item1, T item2) sort;
  final String Function(T item) byValue;
  final List<T> dataList;
  final List<String> indexWordListOrBeforeList;
  //创建索引条回调
  final void Function(String str) indexBarCallBack;
  final Color indexTextColor;
  final ImageProvider<Object> assetImg;
...
class _IndexBarState<T> extends State<IndexBar<T>> {
...
//    1、根据实际数据显示右侧bar
    // _index_word.add('🔍');
    // _index_word.add('☆');
    assetImg = widget.assetImg ?? AssetImage('images/气泡.png');
    if (null != widget.indexTextColor) {
      _textColor = widget.indexTextColor;
    }

    if (null != widget.indexWordListOrBeforeList) {
      _index_word.addAll(widget.indexWordListOrBeforeList);
    }
    if (widget.dataList.isNotEmpty && widget.dataList.length > 0) {
      _listDatas.addAll(widget.dataList);
      if (null != widget.sort) {
        //排序!
        _listDatas.sort((T a, T b) {
          return widget.sort(a, b);
        });
      }

//经过循环，将每一个头的首字母放入index_word数组
      if (null != widget.byValue) {
        for (int i = 0; i < _listDatas.length; i++) {
          if (i < 1 ||
              widget.byValue(_listDatas[i]) !=
                  widget.byValue(_listDatas[i - 1])) {
            _index_word.add(widget.byValue(_listDatas[i]));
          }
        }
      }
    }
